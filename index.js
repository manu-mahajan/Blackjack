...
function startGame() {
  if (roundOver === true) {
    ...
    for (di = 0; di < 2; di++) {
      //Loops twice, giving player and dealer 2 cards each
...
function renderGame() {
  ...
  if (roundOver === true) {
    ...
    // If round is active light up the play buttons and gray out the bet buttons
    ...
    if (playerHand.length === 2) {
      //Only activate the Double Down button if the player is on their first turn
  ...
  for (i = 0; i < playerHand.length; i++) {
    //This loop loads and prints out the card values from the array as well as loading the card images
  ...
  for (i = 0; i < cardBlanks; i++) {
    //This loop reloads the blank/unused card spaces
  ...
  if (isDealerTurn === true) {
    //Only render all cards if it's the dealer turn. Otherwise only display first card
  ...
  document.getElementById("totalamountwon").textContent =
    totalAmountWon.toFixed(2);
  document.getElementById("totalamountlost").textContent =
    totalAmountLost.toFixed(2);
...
function sumCards() {
  ...
  for (si = 0; si < pHandVal.length; si++) {
    //Loop for the total number of cards in player's hand
    ...
    if (value === "1" || value === "J" || value === "Q" || value === "K") {
      ...
      // Otherwise, add the face value of the card to the player sum.
      playerSum += parseInt(value); //parseInt converts the number to an integer
    }
  }
  if (pHandAces > 0) {
    //If there is an ace in the hand, run this logic
    if (playerSum + 11 + (pHandAces - 1) > 21) {
      //Checks to see if one of the aces being 11 would push the total over 21
      playerSum += pHandAces; // If it does push it over 21, only add the total number of aces to the total
    } else {
      //If the ace being 11 doesn't push it over, add 11 plus the number of aces minus 1 (the one you made 11)
  ...
  for (si = 0; si < dHandVal.length; si++) {
    //Loop for the total number of cards in dealer's hand
    ...
    if (value === "1" || value === "J" || value === "Q" || value === "K") {
      ...
      // Otherwise, add the face value of the card to the dealer sum.
  ...
  if (isDealerTurn === false) {
    ...
    if (value === "A") {
      //If the card is an ace show value of 11
      dealerSumShow += 11;
    } else if (
      value === "1" ||
      value === "J" ||
  ...
  if (playerSum < 21 && isDealerTurn === false) {
    if (playerHand.length === 2) {
      //If it's the initial draw, offer double down
      message = "HIT, STAND, or DOUBLE DOWN?";
    } else {
      //If it's after the initial draw, don't offer double down
  ...
  if (roundOver === true) {
    if (playerSum > dealerSum) {
      //Player's total is greater than dealer. Player wins
      message = "You win!";
      roundOver = true; //Mark round as over
      ...
      //Dealer's total is greater than dealer. Dealer wins.
      message = "You lose!";
      roundOver = true; //Mark round as over
      totalAmountLost += bet; //Add bet to total amount lost
      ...
      //Totals are the same. It's a push/tie.
...
function hitCard() {
  if (roundOver === false) {
    //Only allows button to be clicked if Round is active
...
function standCard() {
  if (roundOver === false) {
    ...
    if (dealerSum < dealerDrawTo) {
      //checks to see whether dealerSum is less than the amount the dealer draws to
      cardID = getRandomCard(); //calls random card function and returns the response to cardID
      dealerHand.push(cardStack[cardID]);
      cardStack.splice(cardID, 1); //Puts card in dealer's hand and removes it from the cardStack
      renderGame(); //returns to renderGame
    } else {
      //If the dealer has hit or exceeded the draw to limit we mark the round as over and send it to rendergame
      roundOver = true;
      renderGame();
    }
  }
